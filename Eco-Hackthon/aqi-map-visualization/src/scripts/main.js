    ...
    mockAQIData.forEach(point => {
        ...
        marker.bindPopup(`
            <div style="font-size: 11px;">
                ...
                AQI: <strong style="color: ${color};">${point.aqi}</strong><br>
                ${point.category}<br>
                ● ${point.quality}
            </div>
        `, { maxWidth: 200 });
...
function analyzeRoute(route) {
    ...
    if (nearestPoints.length > 0) {
        detailsHTML += `<strong style="color: #667eea; font-size: 10px;">${nearestPoints.length} monitoring points detected</strong><br>`;
        nearestPoints.slice(0, 8).forEach((point, idx) => {
            ...
            detailsHTML += `<div class="route-waypoint">${idx + 1}. ${point.name}<span class="aqi-badge ${badge}">${point.aqi}</span> <span style="font-size: 9px; color: #999;">(${distanceText}m)</span></div>`;
        });
...
function clearAll() {
    ...
    map.setView(NAGPUR_CENTER, NAGPUR_ZOOM);
    updateFindRouteButton();
